...
export default {
  ...
  initTestData: function (test, dialect, done) {
    const helpers = this;
    const isSnakeTables = this.isSnakeTables();
    this.initTests({
      dialect: dialect,
      beforeComplete: function (sequelize) {
        test.sequelize = sequelize;
        test.User = test.sequelize.define(
          'User',
          {
        ...
        test.HistoryLog = test.sequelize.define(
          'HistoryLog',
          {
        ...
        test.Parent = test.sequelize.define(
          'Parent',
          {
            parentId: {
              type: Sequelize.INTEGER,
        ...
        test.Child = test.sequelize.define(
          'Child',
          {
            childId: {
              type: Sequelize.INTEGER,
  ...
  clearDatabase: function (sequelize, callback, close) {
    ...
    try {
      dropViews().then(dropTables().then(deleteModelFiles, error)).catch(error);
    } catch (ex) {
  ...
  getDummyCreateTriggerStatement: function (tableName) {
    var statement = {
      mysql:
        'CREATE TRIGGER ' + tableName + '_Trigger BEFORE INSERT ON ' + tableName + ' FOR EACH ROW SET NEW.Id = NEW.Id',
      postgres:
        'CREATE OR REPLACE FUNCTION blah() RETURNS trigger AS $$ BEGIN RETURN NEW; END; $$ LANGUAGE plpgsql; \
                 CREATE TRIGGER "' +
        tableName +
        '_Trigger" AFTER INSERT ON "' +
        tableName +
        '" WHEN (1=0) EXECUTE PROCEDURE blah(1);',
      mssql:
        'CREATE TRIGGER ' + tableName + '_Trigger ON ' + tableName + ' AFTER INSERT AS BEGIN SELECT 1 WHERE 1=0; END;',
      sqlite:
