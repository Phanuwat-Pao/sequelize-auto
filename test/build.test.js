...
import * as SequelizeAuto from '../index.js';
...
describe(helpers.getTestDialectTeaser('sequelize-auto build'), function () {
  ...
  var setupModels = function (self, callback, done) {
    var options = _.extend(
      ...
      testConfig[helpers.getTestDialect()],
      self.sequelize.config,
    );

    var auto = new SequelizeAuto(
      self.sequelize.config.database,
      self.sequelize.config.username,
      self.sequelize.config.password,
      options,
    );
      ...
      setupModels(
        ...
        function (tableData) {
          ...
          if (helpers.getTestDialect() === 'sqlite') {
            ...
            expect(fkUsers.id).to.include.keys([
              'source_schema',
              'source_table',
              'source_column',
              'target_schema',
            ...
            expect(fkHistoryLogs.id).to.include.keys([
              'source_schema',
              'source_table',
              'source_column',
              'target_schema',
            ...
            expect(fkParanoidUsers.id).to.include.keys([
              'source_schema',
              'source_table',
              'source_column',
              'target_schema',
            ...
            expect(fkParents.ParentId).to.include.keys([
              'source_schema',
              'source_table',
              'source_column',
              'target_schema',
            ...
            expect(fkKids.ChildId).to.include.keys([
              'source_schema',
              'source_table',
              'source_column',
              'target_schema',
          ...
          expect(fkParanoidUsers.UserId).to.include.keys([
            'source_schema',
            'source_table',
            'source_column',
            'target_schema',
