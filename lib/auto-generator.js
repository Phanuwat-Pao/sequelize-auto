...
import { makeIndent, makeTableName, pluralize, qNameJoin, qNameSplit, recase, singularize, } from './types.js';
...
export class AutoGenerator {
    ...
    generateText() {
        ...
        tableNames.forEach((table) => {
            ...
            if (this.options.lang === 'ts') {
                ...
                str += `export class #TABLE# extends Model<Sequelize.InferAttributes<#TABLE#${omit}>, Sequelize.InferCreationAttributes<#TABLE#${omit}>> {\n`;
                str += this.addTypeScriptModelFields(table);
                str += '\n' + associations.str;
                str += '\n' + this.space[1] + 'declare static associations: {';
                associations.omit.forEach((o, i) => {
                    str += '\n' + this.space[2] + `${o}: Sequelize.Association<#TABLE#, ${associations.associations[i]}>;`;
                });
                str += '\n' + this.space[1] + '};';
    ...
    addTypeScriptAssociationMixins(table) {
        ...
        this.relations.forEach((rel) => {
            if (!rel.isM2M) {
                if (rel.childTable === table) {
                    ...
                    omit.push(rel.parentProp);
                    associations.push(rel.parentModel);
                    ...
                    if (rel.isOne) {
                        ...
                        str += `${sp}${rel.childProp}?: Sequelize.NonAttribute<${rel.childModel}>;\n`;
                        omit.push(rel.childProp);
                        associations.push(rel.childModel);
                ...
                if (rel.parentTable === table) {
                    ...
                    omit.push(lotherModelPlural);
                    associations.push(otherModel);
        ...
        return { needed, str, omit, associations };
    }
    addTypeScriptFields(table, isInterface) {
        ...
        fields.forEach((field) => {
            if (!this.options.skipFields || !this.options.skipFields.includes(field)) {
                ...
                str += `${sp}${name}?: ${this.getTypeScriptType(table, field)} ${isOptional ? '| null' : ''} | undefined | ${this.getTypeScriptType(table, field)}[];\n`;
            }
        });
        return str;
    }
    addTypeScriptModelFields(table) {
        const sp = this.space[1];
        const fields = _.keys(this.tables[table]);
        let str = '';
        fields.forEach((field) => {
            if (!this.options.skipFields || !this.options.skipFields.includes(field)) {
                ...
                str += `${sp}declare ${name}${isOptional ? '?' : ''}: ${this.tables[table][field]['primaryKey']
    ...
    isTimestampField(field) {
        ...
        return ((!additional.createdAt && recase('c', field) === 'createdAt') ||
