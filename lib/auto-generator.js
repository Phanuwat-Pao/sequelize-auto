            ...
            if (this.options.lang === 'ts') {
                ...
                if (primaryKeys.length) {
                    str += `export type #TABLE#Pk = ${primaryKeys
                        .map((k) => `"${(0, types_1.recase)(this.options.caseProp, k)}"`)
                        .join(' | ')};\n`;
    ...
    addTypeScriptFields(table, isInterface) {
        ...
        fields.forEach((field) => {
            if (!this.options.skipFields || !this.options.skipFields.includes(field)) {
                ...
                str += `${sp}${name}${isOptional ? '?' : notNull}: ${this.getTypeScriptType(table, field)}${isOptional ? ' | null' : ''};\n`;
            }
        });
        return str;
    }
    addTypeScriptModelFields(table) {
        ...
        fields.forEach((field) => {
            if (!this.options.skipFields || !this.options.skipFields.includes(field)) {
                ...
                str += `${sp}declare ${name}${isOptional ? '?' : ''}: ${isOptional ? 'Sequelize.CreationOptional<' + type + '>' : type};\n`;
            }
        });
        return str;
    }
